...
def proc2(pipe,q):
    while True:
        print u'proc2 received:',pipe.recv()
        time.sleep(1)
 
def proc3(pipe):
    while True:
        print u'proc3 received:',pipe.recv()
        time.sleep(1)

if __name__ == '__main__':

    print ('sys.getdefaultencoding(): %s \n'%sys.getdefaultencoding()) 
# Build a pipe
    pipe = multiprocessing.Pipe()
    "提示类型错误"
    #print ('pipe: %s'%pipe)
    "print format所有类型都可以打印"
    print 'pipe : {} \n'.format(pipe) 
...
# Pass the other end of the pipe to process 2
    ...
    p4 = Process(target=proc2,args=(pipe[1],q))
